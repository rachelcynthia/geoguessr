...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    // Get profile score total for the user
    ...
    res.json({
      ...
      global_rank: globalRankQuery.rows.length
        ? globalRankQuery.rows[0].rank
        : null,
      country_rank: countryRankQuery.rows.length
        ? countryRankQuery.rows[0].rank
        : null,
...
app.get("/api/leaderboard", async (req, res) => {
  try {
    const result = await db.query(`
      ...
      ORDER BY avg_distance ASC NULLS LAST
      LIMIT 50
    `);
